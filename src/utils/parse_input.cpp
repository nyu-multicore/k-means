  ...
  while(std::getline(input, line)) {
    ...
    for (std::string token; iss >> token;) {
      try {
        ...
        std::cout << "Error on input line " << line_num + 1 << ": invalid number \"" << token << "\"" << std::endl;
        exit(1);
      }
    }
    ...
    if (vector_dim == 0) {
      ...
      std::cout << "Error on input line " << line_num + 1 << ": vector has " << vector.size() << " dimensions, but previous vectors have " << vector_dim << " dimensions" << std::endl;
      exit(1);
    }
